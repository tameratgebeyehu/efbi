...
function ProjectRecord({ submission, title }: { submission: ProjectSubmission; title: string }) {
  return <div className="submitted-project"><section><h2>{title}</h2><dl><div><dt>Problem</dt><dd>{submission.problemStatement}</dd></div><div><dt>Intended users</dt><dd>{submission.intendedUsers}</dd></div><div><dt>Solution</dt><dd>{submission.solutionSummary}</dd></div><div><dt>Reflection</dt><dd>{submission.reflection}</dd></div><div><dt>AI use</dt><dd>{submission.aiUseDisclosure}</dd></div></dl></section><aside><h2>Evidence links</h2>{submission.evidence.filter(Boolean).map((link) => <a key={link} href={link} target="_blank" rel="noopener noreferrer">Open evidence <span>↗</span></a>)}<div className="submission-lock-note"><Icon name="shield" /><p><strong>Immutable submission</strong><span>This submitted version cannot be edited or replaced.</span></p></div></aside></div>
}
...
export function SubmissionPage() {
  ...
  if (state === 'loading') return <section className="submission-page"><div className="shell submission-state"><div className="submission-spinner" /><h1>Checking your project access…</h1></div></section>
  if (state === 'error') return <section className="submission-page"><div className="shell submission-state"><h1>Your project workspace could not open.</h1><p>Check your connection and try again.</p><button className="button button--primary" onClick={() => window.location.reload()}>Retry</button></div></section>
  if (state === 'ineligible') return <section className="submission-page"><div className="shell submission-state"><Icon name="shield" /><p className="eyebrow-label">Project locked</p><h1>Finish every lesson first.</h1><p>Your progress in this course version must be complete before EFBI accepts a project draft.</p><Link className="button button--primary" to={`/learn/${routeCourseId}`}>Continue learning</Link></div></section>
  ...
  if (locked && submission) {
    ...
    return <section className="submission-page"><div className="shell"><header className="submission-hero"><div><p className="eyebrow-label">{course?.courseTitle} · {showingRevision ? 'Project revision submitted' : 'Project submitted'}</p><h1>{current.projectTitle}</h1><p>Submitted {submittedDate(current.submittedAt)}. {currentResult ? 'Your human review is complete.' : 'Your work is locked while EFBI assigns and completes a review.'}</p></div><span className="submission-status submission-status--submitted">{currentResult ? 'Reviewed' : 'Submitted'}</span></header>{currentResult && <LearnerReviewPanel result={currentResult} isRevision={showingRevision} />}{currentResult?.decision === 'approved' && user && course && <CertificateRequestPanel uid={user.uid} defaultName={user.displayName ?? ''} result={currentResult} course={course} />}<ProjectRecord submission={current} title={showingRevision ? 'Revision · Version 2' : 'Your project record · Version 1'} />{showingRevision && <details className="version-history"><summary><span><strong>Version history</strong><small>Open the preserved original submission and first review</small></span><span>Version 1</span></summary>{reviewResult && <LearnerReviewPanel result={reviewResult} />}<ProjectRecord submission={submission} title="Original submission · Version 1" /></details>}{message && <p className="form-status form-status--success">{message}</p>}<Link className="button button--outline" to="/account">Back to account</Link></div></section>
  }

  return <section className="submission-page"><div className="shell">
