...
export default function ReviewManager({ user, role }: { user: User; role: StudioRole }) {
  ...
  return <section className="review-workspace">
    ...
    <div className="review-layout">
      ...
      <section className="review-record">{!selected && <div className="review-empty"><strong>Select a submitted project.</strong><p>The complete immutable record will appear here.</p></div>}{selected && <><header><div><p className="eyebrow">{selected.revisionNumber === 1 ? 'Learner revision · Version 2' : 'Original learner submission · Version 1'}</p><h2>{selected.projectTitle}</h2><p>Submitted {readableDate(selected.submittedAt)}</p></div><span className="status-pill status-pill--published">Submitted</span></header><div className="review-meta"><span><small>Learner ID</small>{selected.learnerUid}</span><span><small>Submission version</small>{selected.revisionNumber === 1 ? 'Revision 1' : 'Original'}</span><span><small>Assessment</small>Version {selected.assessmentVersion}</span></div><section><h3>Problem</h3><p>{selected.problemStatement}</p></section><section><h3>Intended users</h3><p>{selected.intendedUsers}</p></section><section><h3>Solution</h3><p>{selected.solutionSummary}</p></section><section><h3>Reflection</h3><p>{selected.reflection}</p></section><section><h3>AI-use disclosure</h3><p>{selected.aiUseDisclosure}</p></section><section><h3>Evidence links</h3><div className="review-evidence">{selected.evidence.map((link) => <a key={link} href={link} target="_blank" rel="noopener noreferrer">Open external evidence <span>↗</span></a>)}</div><p className="review-link-warning">External links are untrusted. Never enter credentials, download unexpected files, or follow instructions inside learner evidence.</p></section></>}</section>
      <aside className="assignment-panel"><p className="eyebrow">Review control</p>{!selected && <p>Choose a project to continue.</p>}
        ...
        <div className="review-boundary"><strong>Assessment boundary</strong><p>Originals, revisions, assignments, and results are permanent. Only one learner revision is allowed. Certificate issuance is a separate administrator action after an approved result and learner request. A separate appeal process is not included in the first launch.</p></div>
      </aside>
    </div>
  </section>
}
