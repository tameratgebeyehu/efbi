      ...
      <div className="course-layout lesson-layout-admin">
        ...
        <form className="course-editor lesson-editor" onSubmit={(event) => void saveLesson(event, 'draft')}>
          ...
          <div className="field-grid">
            ...
            <label>YouTube video<span>Paste a full YouTube link or the 11-character video ID.</span><input value={form.videoYoutubeId} onChange={(event) => updateField('videoYoutubeId', event.target.value.trim())} placeholder="https://youtu.be/..." inputMode="url" />{form.videoYoutubeId.trim() && normalized.content.videoYoutubeId && <small className="field-success">Video recognized · {normalized.content.videoYoutubeId}</small>}</label>
          </div>
