...
export default function ProgramManager({ user }: { user: User }) {
  ...
  return <section className="course-workspace program-workspace">
    <header className="workspace-title"><div><p className="eyebrow">Website content</p><h1>Programs</h1><p>Edit the learning areas visitors see on the website.</p></div><button className="primary-action" onClick={startNew}>New program</button></header>
    ...
    <div className="course-layout">
      ...
      <form className="course-editor" onSubmit={submit}>
        <div className="editor-heading"><div><p className="eyebrow">{selected ? `Saved version ${selected.revision}` : 'New program'}</p><h2>{selected?.title ?? 'Create a program'}</h2></div>{selected && <StatusPill status={selected.status} />}</div>
      ...
      <aside className="course-preview"><p className="eyebrow">Safe preview</p><OwnerPreviewLauncher item={{ kind: 'program', id: selected?.programId ?? programId.trim(), content: normalized.content }} issues={normalized.errors} /><article className={`program-admin-preview accent-${normalized.content.accent}`}><span className="program-admin-preview__short">{normalized.content.shortTitle || 'EFBI'}</span><small>{normalized.content.level} · {normalized.content.durationWeeks || 0} weeks</small><h2>{normalized.content.title || 'Program title'}</h2><p>{normalized.content.description || 'The public program description appears here.'}</p><strong>Learning outcome</strong><p>{normalized.content.outcome || 'The learner outcome appears here.'}</p></article>
        ...
        {selected?.status === 'ready' && <div className="publish-panel"><strong>Approved and ready</strong><p>Publishing updates this program on the public website.</p>{hasChanges ? <p className="publish-warning">Save or discard unsaved changes first.</p> : <label className="confirm-check"><input type="checkbox" checked={confirmPublish} onChange={(event) => setConfirmPublish(event.target.checked)} />I checked the program preview.</label>}<button className="publish-action" type="button" onClick={() => void publish()} disabled={busy || hasChanges || !confirmPublish}>Publish program</button></div>}
      </aside>
    </div>
  </section>
}
