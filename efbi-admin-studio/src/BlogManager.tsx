...
function StatusPill({ status }: { status: BlogStatus }) {
  return <span className={`status-pill status-pill--${status}`}>{status === 'ready' ? 'Approved' : status === 'draft' ? 'Draft' : status === 'published' ? 'Published' : 'Hidden'}</span>
}
...
export default function BlogManager({ user }: { user: User }) {
  ...
  return <section className="course-workspace blog-workspace-admin">
    <header className="workspace-title"><div><p className="eyebrow">Website content</p><h1>Blog</h1><p>Write an article, check its preview, and choose when it appears publicly.</p></div><button className="primary-action" onClick={startNew}>New article</button></header>
    ...
    <div className="course-layout blog-layout-admin">
      ...
      <form className="course-editor" onSubmit={submit}><div className="editor-heading"><div><p className="eyebrow">{selected ? `Saved version ${selected.revision}` : 'New article'}</p><h2>{selected?.title ?? 'Create an article'}</h2></div>{selected && <StatusPill status={selected.status} />}</div>
        ...
        <div className="editor-actions">{!selected && <button className="primary-action" type="submit" disabled={busy || normalized.errors.length > 0}>{busy ? 'Creating…' : 'Create article'}</button>}{selected && <><button type="button" onClick={() => void writeDraft('draft')} disabled={busy || normalized.errors.length > 0 || (selected.status !== 'ready' && !hasChanges)}>Save changes</button><button className="primary-action" type="button" onClick={() => void writeDraft('ready')} disabled={busy || normalized.errors.length > 0 || (selected.status === 'ready' && !hasChanges)}>Approve</button></>}</div>
      </form>
      <aside className="course-preview blog-preview-admin"><p className="eyebrow">Article preview</p><OwnerPreviewLauncher item={{ kind: 'article', id: selected?.postId ?? postId.trim(), content: normalized.content }} issues={normalized.errors} /><article className={`blog-admin-card tone-${normalized.content.tone}`}><span>{normalized.content.category || 'Category'}</span>{normalized.content.featured && <small>Featured article</small>}<h2>{normalized.content.title || 'Article title'}</h2><p>{normalized.content.excerpt || 'The article excerpt appears here.'}</p><div className="blog-admin-meta">{normalized.content.authorName || 'Author'} · {normalized.content.readingMinutes || 0} min read</div><div className="blog-admin-body">{normalized.content.bodyMarkdown || 'The full article appears here.'}</div></article>
        ...
        {selected?.status === 'ready' && <div className="publish-panel"><strong>Approved and ready</strong><p>Publishing makes this article visible on the EFBI blog.</p>{hasChanges ? <p className="publish-warning">Save or discard unsaved changes first.</p> : <label className="confirm-check"><input type="checkbox" checked={confirmPublish} onChange={(event) => setConfirmPublish(event.target.checked)} />I checked the article preview.</label>}<button className="publish-action" type="button" onClick={() => void publish()} disabled={busy || hasChanges || !confirmPublish}>Publish article</button></div>}
